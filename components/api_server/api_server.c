...
"<style>:root{--bg:#f6f8f8;--panel:#fff;--text:#111827;--muted:#64748b;--line:#d7e0df;--accent:#12b8b2;--green:#188038;--blue:#1d6fd1;--red:#c9352b}body.dark{--bg:#071014;--panel:#0d171d;--text:#f3f7f8;--muted:#9aa8b6;--line:#25333b}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:system-ui,-apple-system,Segoe UI,sans-serif}.wrap{max-width:1180px;margin:auto;padding:20px}.top,.panel,.card,.notice{background:var(--panel);border:1px solid var(--line);border-radius:8px}.top{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:22px 24px;margin-bottom:14px}.brand{display:flex;align-items:center;gap:16px}.mark{width:48px;height:48px;border-radius:8px;background:var(--accent);color:#fff;display:grid;place-items:center;font-weight:800}h1{margin:0;font-size:30px;line-height:1.1}h2{margin:0 0 14px;font-size:15px;text-transform:uppercase;letter-spacing:.02em}.sub,.muted{color:var(--muted)}.right{text-align:right}.status{color:var(--green);font-weight:700;margin-bottom:8px}button{font:inherit;min-height:42px;border:1px solid var(--accent);background:transparent;color:var(--accent);border-radius:7px;padding:10px 14px;cursor:pointer}button[disabled]{opacity:.55;cursor:wait}.danger{background:var(--red);border-color:var(--red);color:#fff}.panel{padding:20px;margin-bottom:14px}.notice{display:none;padding:12px 14px;margin-bottom:14px}.notice.show{display:block}.notice.ok{border-color:var(--green)}.notice.err{border-color:var(--red)}.nav{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}.nav button.on{background:var(--accent);color:#fff}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:14px}.card{padding:18px;min-height:138px}.metric{font-size:46px;font-weight:800;color:var(--blue);line-height:1}.cam{overflow:hidden}.cam img{display:block;width:100%;max-height:520px;object-fit:contain;background:#050b11;border:1px solid var(--line);border-radius:8px}.camrow{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-top:12px}.manual{display:grid;grid-template-columns:repeat(5,1fr);gap:12px}.run{min-height:100px;border-color:var(--green);color:var(--green);font-weight:700}.run b{display:block;font-size:34px}.slot{display:grid;grid-template-columns:70px 120px 130px 1fr 90px;gap:10px;align-items:end;border-top:1px solid var(--line);padding-top:12px;margin-top:12px}.days{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}.days label{margin:0;text-align:center;font-size:12px}.addtime{margin-top:14px}label{display:block;margin:10px 0 6px;color:var(--muted);font-weight:600}input,select,textarea{width:100%;border:1px solid var(--line);border-radius:7px;padding:10px;background:var(--panel);color:var(--text);font:inherit}input[type=checkbox]{width:auto}canvas{width:100%;height:220px;border:1px solid var(--line);border-radius:8px}.actions{display:flex;align-items:center;justify-content:space-between;gap:12px}.term{background:#050b11;color:#d1fae5;border-color:#17212b}.term pre{font-family:Consolas,monospace;white-space:pre-wrap;overflow:auto;max-height:430px}.cmd{display:flex;gap:8px}.cmd input{font-family:Consolas,monospace}@media(max-width:900px){.grid,.manual{grid-template-columns:1fr 1fr}.slot{grid-template-columns:1fr}.top{align-items:flex-start}.right{text-align:left}}@media(max-width:560px){.wrap{padding:10px}.grid,.manual{grid-template-columns:1fr}.top{padding:16px}h1{font-size:24px}.metric{font-size:40px}}</style></head>"
...
"<nav class=nav><button data-p=dash onclick=\"show('dash')\">Status</button><button data-p=manual onclick=\"show('manual')\">Manual</button><button data-p=schedule onclick=\"show('schedule')\">Schedule</button><button data-p=weather onclick=\"show('weather')\">Weather</button><button data-p=sensor onclick=\"show('sensor')\">Sensors</button><button data-p=term onclick=\"show('term')\">Console</button></nav><div id=notice class=notice></div>"
"<section id=dash></section><section id=manual hidden></section><section id=schedule hidden></section><section id=weather hidden></section><section id=sensor hidden></section><section id=term hidden></section></main>"
"<script>let status={},settings={},moisture=[],latest={},diag=[],active='dash',livePause=0,scheduleDirty=false,scheduleDraft=null;const secs=[300,600,900,1200],names=['sun','mon','tue','wed','thu','fri','sat'],labels=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];if(!localStorage.pin)localStorage.pin=prompt('Local PIN','1234')||'';document.body.className=localStorage.theme||'';"
"async function api(p,o){let r=await fetch(p,{headers:{'Content-Type':'application/json','X-Local-PIN':localStorage.pin||''},...(o||{})});let body=r.headers.get('content-type')?.includes('json')?await r.json():await r.text();if(!r.ok)throw new Error(body.error||body||('HTTP '+r.status));return body}async function syncTime(){if(status.time_valid)return;await api('/api/v1/time',{method:'POST',body:JSON.stringify({epoch_sec:Math.floor(Date.now()/1000)})})}function show(id){active=id;if(id=='schedule'&&!scheduleDirty)scheduleDraft=cloneSchedule();for(const s of document.querySelectorAll('section'))s.hidden=s.id!==id;document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('on',b.dataset.p==id));render()}function note(t,k=''){if(k)livePause=Date.now()+5000;notice.className='notice show '+k;notice.textContent=t}function busy(on){document.querySelectorAll('button').forEach(b=>b.disabled=on)}function editingSchedule(){return active=='schedule'&&(scheduleDirty||(document.activeElement&&schedule.contains(document.activeElement)))}async function saveIt(label,fn){try{busy(true);note('Saving '+label+'...');await syncTime();fn();await api('/api/v1/settings',{method:'POST',body:JSON.stringify(settings)});note('Saved '+label+'.','ok');await refresh(true)}catch(e){note('Could not save '+label+': '+e.message,'err')}finally{busy(false)}}"
"async function refresh(force=false){try{let cv=document.getElementById('cmd')?.value||'';[status,settings,moisture,latest,diag]=await Promise.all([api('/api/v1/status'),api('/api/v1/settings'),api('/api/v1/moisture'),api('/api/v1/moisture/latest'),api('/api/v1/diagnostics')]);if(!status.time_valid){await syncTime();status=await api('/api/v1/status')}ensureSlots();let keepCam=active=='dash'&&document.getElementById('camStream')&&!force;if(!keepCam&&(force||!editingSchedule())){if(!scheduleDirty)scheduleDraft=cloneSchedule();render();let ci=document.getElementById('cmd');if(ci){ci.value=cv;let o=document.getElementById('termOut');if(o)o.scrollTop=o.scrollHeight}drawGraph()}liveNote()}catch(e){note('Controller status unavailable: '+e.message,'err')}}function ensureSlots(){if(!settings.schedule_slots)settings.schedule_slots=[{enabled:true,start:settings.watering_windows?.[0]?.start||'06:00',duration_sec:settings.schedule?.duration_sec||120,days:names.slice()}];settings.schedule_slots=settings.schedule_slots.filter(s=>s.enabled&&s.days&&s.days.length&&s.duration_sec>0)}"
"function card(t,b){return `<div class=card><h2>${t}</h2>${b}</div>`}function cameraUrl(path='/stream'){return 'http://esp32-cam.local'+path}function cameraPanel(){return `<div class='panel cam'><h2>Camera monitor</h2><img id=camStream src='${cameraUrl()}' onerror=\"this.style.display='none';document.getElementById('camFallback').hidden=false\"><div id=camFallback class=muted hidden>Camera stream unavailable at esp32-cam.local.</div><div class=camrow><div class=muted>ESP32-CAM live stream</div><button onclick=\"window.open(cameraUrl('/'),'_blank')\">Open camera</button></div></div>`}function minText(s){return s<60?s+' sec':Math.round(s/60)+' min'}function prettyState(){return (status.pump_state||'UNKNOWN').replaceAll('_',' ').toLowerCase()}function weatherText(){if(!settings.weather_adjust_enabled)return 'Off';let m=settings.weather_last_multiplier||1;return `${Math.round(m*100)}% duration, ${settings.weather_last_temp_c||0} C, ${settings.weather_last_precip_mm||0} mm rain`}function liveNote(){if(Date.now()<livePause)return;let running=status.current_run_remaining_sec>0;let res=status.reservoir_bypass?'reservoir check bypassed':(status.reservoir_ok?'reservoir OK':'reservoir low');let msg=running?`Watering now, ${status.current_run_remaining_sec}s remaining.`:`Controller is ${prettyState()}.`;let cls=status.pump_state=='IDLE'&&status.reservoir_ok?'ok':'';note(`${msg} ${res}. ${status.block_reason||'Ready.'}`,cls)}"
"function render(){let mp=latest.percent==null?'--':latest.percent,can=status.pump_state=='IDLE'&&status.reservoir_ok;dash.innerHTML=`<div class=grid>${card('Pump status',`<div class=metric>${status.pump_state||'--'}</div><div class=muted>${status.current_run_remaining_sec||0}s remaining</div><div class=muted>IN1 GPIO${status.pump_gpio}: ${status.pump_gpio_level}, IN2 GPIO${status.pump_gpio_b}: ${status.pump_gpio_b_level}, SLEEP ${status.pump_sleep_gpio>=0?'GPIO'+status.pump_sleep_gpio+': '+status.pump_sleep_gpio_level:'not set'}</div><div class=muted>${status.block_reason||'Ready'}</div>`)}${card('Moisture',`<div class=metric>${mp}%</div><div class=muted>Soil sensor ${settings.moisture_sensor_enabled?'enabled':'disabled'} on GPIO${settings.moisture_gpio}</div>`)}${card('Next watering',`<div class=metric>${can?'Ready':'Hold'}</div><div class=muted>${status.next_scheduled||'No schedule'}</div><div class=muted>Weather: ${weatherText()}</div>`)}</div>${cameraPanel()}<div class='panel actions'><div><h2>Controller feedback</h2><b>${can?'Ready to water':'Watering blocked'}</b><div class=muted>${status.block_reason||'ESP32 state is live; this page refreshes every second.'}</div></div><button onclick=\"show('manual')\">Manual controls</button></div>`;"
...
static esp_err_t status_get(httpd_req_t *req)
{
    ...
    static pump_state_t last_state;
    static bool last_reservoir;
    static char last_block[64];
    scheduler_refresh_next_run();
    const char *next = scheduler_next_run_text();
    if (!seen || runtime.pump_state != last_state || runtime.reservoir_ok != last_reservoir ||
        strcmp(runtime.block_reason, last_block) != 0) {
        char msg[160];
        snprintf(msg, sizeof(msg), "status %s, reservoir %s, block '%s', next %s",
                 pump_state_to_string(runtime.pump_state), runtime.reservoir_ok ? "OK" : "LOW",
                 runtime.block_reason[0] ? runtime.block_reason : "none", next);
        history_store_log_diagnostic(msg);
        seen = true;
        last_state = runtime.pump_state;
        last_reservoir = runtime.reservoir_ok;
        snprintf(last_block, sizeof(last_block), "%s", runtime.block_reason);
    }
    ...
    cJSON_AddStringToObject(root, "next_scheduled", next);
...
static esp_err_t time_post(httpd_req_t *req)
{
    ...
    free(body);
    const cJSON *epoch = cJSON_GetObjectItem(json, "epoch_sec");
    if (!cJSON_IsNumber(epoch) || epoch->valuedouble < 1700000000) {
        cJSON_Delete(json);
        httpd_resp_set_status(req, "400 Bad Request");
        httpd_resp_sendstr(req, "{\"error\":\"valid epoch_sec required\"}");
        return ESP_OK;
    }
    struct timeval tv = {.tv_sec = (time_t)epoch->valuedouble};
    settimeofday(&tv, NULL);
    scheduler_refresh_next_run();
    history_store_log_diagnostic("Controller clock set from browser.");
    cJSON_Delete(json);
...
esp_err_t api_server_start(void)
{
    ...
    reg(server, "/api/v1/time", HTTP_POST, time_post);
