...
"async function refresh(force=false){try{let cv=document.getElementById('cmd')?.value||'';[status,settings,moisture,latest,diag]=await Promise.all([api('/api/v1/status'),api('/api/v1/settings'),api('/api/v1/moisture'),api('/api/v1/moisture/latest'),api('/api/v1/diagnostics')]);if(!status.time_valid){await syncTime();status=await api('/api/v1/status')}ensureSlots();if(force||!editingSchedule()){if(!scheduleDirty)scheduleDraft=cloneSchedule();render();let ci=document.getElementById('cmd');if(ci){ci.value=cv;let o=document.getElementById('termOut');if(o)o.scrollTop=o.scrollHeight}drawGraph()}liveNote()}catch(e){note('Controller status unavailable: '+e.message,'err')}}function ensureSlots(){if(!settings.schedule_slots)settings.schedule_slots=[{enabled:true,start:settings.watering_windows?.[0]?.start||'06:00',duration_sec:settings.schedule?.duration_sec||120,days:names.slice()}];settings.schedule_slots=settings.schedule_slots.filter(s=>s.enabled&&s.days&&s.days.length&&s.duration_sec>0)}"
