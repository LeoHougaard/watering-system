...
"<nav class=nav><button data-p=dash onclick=\"show('dash')\">Status</button><button data-p=manual onclick=\"show('manual')\">Manual</button><button data-p=schedule onclick=\"show('schedule')\">Schedule</button><button data-p=weather onclick=\"show('weather')\">Weather</button><button data-p=sensor onclick=\"show('sensor')\">Sensors</button><button data-p=history onclick=\"show('history')\">History</button><button data-p=wifi onclick=\"show('wifi')\">Wi-Fi</button><button data-p=term onclick=\"show('term')\">Console</button></nav><div id=notice class=notice></div>"
"<section id=dash></section><section id=manual hidden></section><section id=schedule hidden></section><section id=weather hidden></section><section id=sensor hidden></section><section id=history hidden></section><section id=wifi hidden></section><section id=term hidden></section></main>"
"<script>let status={},settings={},history=[],moisture=[],latest={},wifi={},diag=[],active='dash',livePause=0,scheduleDirty=false,scheduleDraft=null;const secs=[300,600,900,1200],names=['sun','mon','tue','wed','thu','fri','sat'],labels=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];if(!localStorage.pin)localStorage.pin=prompt('Local PIN','1234')||'';document.body.className=localStorage.theme||'';"
"async function api(p,o){let r=await fetch(p,{headers:{'Content-Type':'application/json','X-Local-PIN':localStorage.pin||''},...(o||{})});let body=r.headers.get('content-type')?.includes('json')?await r.json():await r.text();if(!r.ok)throw new Error(body.error||body||('HTTP '+r.status));return body}function show(id){active=id;if(id=='schedule'&&!scheduleDirty)scheduleDraft=cloneSchedule();for(const s of document.querySelectorAll('section'))s.hidden=s.id!==id;document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('on',b.dataset.p==id));render()}function note(t,k=''){if(k)livePause=Date.now()+5000;notice.className='notice show '+k;notice.textContent=t}function busy(on){document.querySelectorAll('button').forEach(b=>b.disabled=on)}function editingSchedule(){return active=='schedule'&&(scheduleDirty||(document.activeElement&&schedule.contains(document.activeElement)))}async function saveIt(label,fn){try{busy(true);note('Saving '+label+'...');fn();await api('/api/v1/settings',{method:'POST',body:JSON.stringify(settings)});note('Saved '+label+'.','ok');await refresh(true)}catch(e){note('Could not save '+label+': '+e.message,'err')}finally{busy(false)}}"
"async function refresh(force=false){try{let cv=document.getElementById('cmd')?.value||'';[status,settings,history,moisture,latest,wifi,diag]=await Promise.all([api('/api/v1/status'),api('/api/v1/settings'),api('/api/v1/history'),api('/api/v1/moisture'),api('/api/v1/moisture/latest'),api('/api/v1/wifi'),api('/api/v1/diagnostics')]);ensureSlots();if(force||!editingSchedule()){if(!scheduleDirty)scheduleDraft=cloneSchedule();render();let ci=document.getElementById('cmd');if(ci){ci.value=cv;let o=document.getElementById('termOut');if(o)o.scrollTop=o.scrollHeight}drawGraph()}liveNote()}catch(e){note('Controller status unavailable: '+e.message,'err')}}function ensureSlots(){if(!settings.schedule_slots)settings.schedule_slots=[{enabled:true,start:settings.watering_windows?.[0]?.start||'06:00',duration_sec:settings.schedule?.duration_sec||120,days:names.slice()}];settings.schedule_slots=settings.schedule_slots.filter(s=>s.enabled&&s.days&&s.days.length&&s.duration_sec>0)}"
